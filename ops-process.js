...
// TODO: need github context


async function runOpsProcess(github, core) {
  const payload = github.context.payload
  const opsIssueNodeID = payload.client_payload.command.resource.id
  const numOfPOCDays = payload.client_payload.data['POC Days']
  ...
  try {
    ...
    const opsIssueInfo = await functions.getIssueInfoFromNodeID(opsIssueNodeID)
    ...
    await functions.moveIssueColumn(cardId, inProgressColumnID)

    // Comment on POC Trial issue
    console.log("Commenting on POC Trial Issue")
    await functions.commentOnIssue(trialIssueNodeID, `Trial has been enabled for ${numOfPOCDays} days!`)
 
    // Update the GHAS POC Issue body to include updated expiration
    console.log("Updating on POC Trial Issue with new Metadata")
    const expireDate = getExpireDate(numOfPOCDays)
    var updatedBody = getUpdateMetadataBody(trialIssueInfo.body, expireDate)
    await functions.updateIssueBody(trialIssueNodeID, updatedBody)
...
function getUpdateMetadataBody(body, expireDate) {
...
function getExpireDate(numOfPOCDays) {
  return Date.now() + (numOfPOCDays * 24 * 3600 * 1000)
}

function getIssueMetadataObject(body){
  return JSON.parse(body.match(/<!-- METADATA:.*?({.*}).*-->/)[1])
}

function getProjectCard(trialProjectCardNodes){
  var cardId = ''
...
async function getTrialIssueInfo(trialIssueNodeID){
  const trialIssueInfo = await functions.getIssueInfoFromNodeID(trialIssueNodeID) 
